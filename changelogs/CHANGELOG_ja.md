...
- **ソート無効化オプション**
  - 依存関係ソートを行わずファイル順序でissueを作成する`--no-sort`オプションを追加
  - ファイル順序を依存関係制約より優先したいワークフローで依存関係解決をバイパス可能
  - 入力ファイルの順序を依存関係制約より優先するワークフローに有用
  - 存在しない依存関係を持つissueの作成を防ぐため、issueの参照は引き続き検証

### 改善

- **依存関係ソートの強化**
  - 依存関係解決を`DependsOn`関係のみに簡素化
  - より明確な結果のため`Blocks`と`Related`関係を順序計算から除外
  - 依存関係レベル内でのファイル順序保持を改善
  - 元のファイル順序を第一基準とする安定トポロジカルソートの実装
  - issueの予測可能性と直感的な順序付けの向上

### 修正

- **ファイル順序の安定性**
  - INFRA-001〜004などの連続するissueが分離される問題を修正
  - 依存関係が許可する場合にファイル順序を維持する安定ソートアルゴリズムの強化
  - 論理的にグループ化されたissueの予期しない再配置を削減
  - より予測可能なissue作成順序によるユーザーエクスペリエンスの向上

### 技術的詳細

- 位置ベースの安定ソートを使用するように依存関係リゾルバを修正
- ファイル順序の優先順位を維持するためのレベル毎の処理を実装
- ソート無効化機能のためのGetOriginalOrder()メソッドを追加
- ソート無効化モードでの参照検証と依存関係サイクル検出の分離
